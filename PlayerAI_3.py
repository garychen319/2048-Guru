...
class PlayerAI(BaseAI):
    ...
    def decision(grid):
        ...
        initialDepth = 2
        prevState = PlayerAI.maximize(grid, alpha, beta, initialDepth, timeout)
        while time.time() < timeout:
            initialDepth += 1
            currState = PlayerAI.maximize(grid, alpha, beta, initialDepth, timeout)
            if currState[0] == -1:
                return prevState[0]
            prevState = currState
        return currState[0]

    def minimize(grid, alpha, beta, depth, timeout):
        util = 0
        ...
        for cell in grid.getAvailableCells():
            ...
            for val in possibleVals:
                child = grid.clone()
                child.setCellValue(cell, val)
                util = PlayerAI.maximize(child, alpha, beta, depth, timeout)[1]

                if time.time() > timeout:
                    return (None, PlayerAI.getUtility(grid))
    ...
    def maximize(grid, alpha, beta, depth, timeout):
        direction = -1
        util = 0
        # if time.time() > timeout:
        ...
        depth -= 1
        if depth == 0:  # stop (terminal test)
            return (-1, PlayerAI.getUtility(grid))

        for movenum in grid.getAvailableMoves():
            child = grid.clone()
            child.move(movenum)

            util = PlayerAI.minimize(child, alpha, beta, depth, timeout)
    ...
    def getUtility(grid):
        ...
        for n in range(0, 7):
            ...
            for i in range(0, 4):
                for j in range(0, 4):
                    sum += (grid.map[i][j] * GRADIENT[n][i][j])
            utility.append(sum)
